...
class _TutorScreenState extends ConsumerState<TutorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<TutorState>(tutorProvider, (previous, next) {
      ...
      if (nextTransition != null && !identical(previousTransition, nextTransition)) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _advanceAfterTransition(PendingPracticeTransition transition) {
    ...
    final nextLesson = lessons[transition.nextLessonIndex!];
    ref
        .read(keyboardLayoutProvider.notifier)
        .setLayout(_layoutForLesson(nextLesson));

    if (transition.isLessonAdvance) {
      ref.read(tutorProvider.notifier).selectLesson(transition.nextLessonIndex!);
    } else {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 8),
          _buildFilterChip(LessonFilter.phonetic, 'অভ্র', Icons.text_fields, Colors.green),
        ],
      ),
    );
  }
