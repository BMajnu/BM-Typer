...
import 'package:bm_typer/core/constants/layouts/phonetic_layout.dart';
...
class TypingGuide extends ConsumerWidget {
              ...
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: Text(
                      _getGuideDisplayCharacter(char),
                      style: TextStyle(
                        fontSize: 24,
  ...
  String _getGuideDisplayCharacter(String char) {
    if (char == ' ') return '␣';
    return char;
  }

  bool _isEnglishLetter(String symbol) {
    return symbol.length == 1 && RegExp(r'[A-Za-z]').hasMatch(symbol);
  }

  String _formatKeyToken(String symbol) {
    if (symbol == '`') {
      return '` (Backtick)';
    }

    if (_isEnglishLetter(symbol) &&
  ...
  String _formatPhoneticSequenceInline(String sequence) {
    return sequence.split('').map(_formatKeyToken).join(' + ');
  }

  (String, String) _buildSingleKeyGuidance(
    String key,
    String output, {
    ...
    final fingerInfo =
        _getFingerForCharacter(_isEnglishLetter(key) ? key.toLowerCase() : key);
    final fingerNameBn = _getBengliFinger(fingerInfo.finger);
    final handNameBn = _getBengliHand(fingerInfo.hand);
    final renderedKey = _formatKeyToken(key);
    final renderedOutput = outputLabel ?? output;

    return (
      '$handNameBn $fingerNameBn | $renderedKey কী চাপুন ($layoutLabel)',
      '$renderedOutput → ${_formatPhoneticSequenceInline(key)}',
    );
  }
  ...
  String _formatPhoneticSequence(String sequence) {
    return sequence.split('').map(_formatKeyToken).join(', তারপর ');
  }

  (String, String)? _buildOfficialPhoneticSequenceGuidance(String character) {
    final sequences = PhoneticKeyboardLayout.getSequencesForOutput(character);
    if (sequences.isEmpty) return null;

    final preferredSequence =
        PhoneticKeyboardLayout.getPrimarySequenceForOutput(character);
    if (preferredSequence == null) return null;

    final alternatives = sequences
        .where((sequence) => sequence != preferredSequence)
        .take(2)
        .toList();

    final String fingerGuidance;
    if (preferredSequence.length == 1) {
      fingerGuidance = _buildSingleKeyGuidance(
        preferredSequence,
        character,
        layoutLabel: 'অভ্র',
        outputLabel: character == '্' ? 'হসন্ত' : null,
      ).$1;
    } else {
      fingerGuidance =
          'ধাপে ধাপে | ${_formatPhoneticSequence(preferredSequence)} চাপুন (অভ্র)';
    }

    var keyGuidance =
        'Official অভ্র/Windows: ${character == '্' ? 'হসন্ত' : character} → ${_formatPhoneticSequenceInline(preferredSequence)}';
    if (alternatives.isNotEmpty) {
      keyGuidance +=
          ' | বিকল্প: ${alternatives.map(_formatPhoneticSequenceInline).join(', ')}';
    }

    if (character == 'থ') {
      keyGuidance +=
          ' | নোট: থ লিখতে ছোট হাতের th ব্যবহার করুন; ঠ লিখতে Shift + T নয়, Th ব্যবহার হয়।';
    }
  ...
  (String, String) _getPhoneticGuidance(String character) {
    if (character == ' ') {
      return ('বৃদ্ধাঙ্গুলি | স্পেসবার চাপুন', 'স্পেস → Space Bar');
    }

    final officialSequenceGuidance =
        _buildOfficialPhoneticSequenceGuidance(character);
    if (officialSequenceGuidance != null) {
      return officialSequenceGuidance;
    }

    return ('সঠিক আঙ্গুল ব্যবহার করুন (অভ্র)', 'কীবোর্ড নয়, স্ক্রিনে তাকান');
  }
}
