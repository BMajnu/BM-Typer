...
class OrganizationService {
  ...
  Future<bool> addMember({
    ...
    try {
      ...
      final userQuery = await _firestore
          .collection('users')
          .where('profile.email',
              isEqualTo: email) // Check nested profile first
          .limit(1)
          .get();
  ...
  /// Ensure privileged users have a membership document so Firestore rules
  /// allow them to read the org members subcollection.
  Future<void> ensurePrivilegedMemberAccess({
    required OrganizationModel org,
    required UserModel user,
  }) async {
    final isOrgOwner = org.adminUserId == user.id;
    final isPrivilegedRole =
        user.role == UserRole.orgAdmin || user.role == UserRole.teamLead;

    if (!isOrgOwner && !isPrivilegedRole) {
      return;
    }

    if (user.id.isEmpty || org.id.isEmpty) {
      return;
    }
    ...
    final hasJoinedAt = memberData != null && memberData['joinedAt'] != null;
    ...
    final memberPayload = <String, dynamic>{
      ...
      'isActive': true,
      'role':
          isOrgOwner || user.role == UserRole.orgAdmin ? 'admin' : 'team_lead',
      'isTeamLead': user.role == UserRole.teamLead,
  ...
  Future<bool> assignTeamLead({
    ...
    try {
      ...
      await _firestore.collection('users').doc(userId).update({
        'profile.role':
            isTeamLead ? UserRole.teamLead.name : UserRole.student.name,
      });
