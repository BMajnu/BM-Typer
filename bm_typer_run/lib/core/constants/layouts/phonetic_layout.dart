...
class PhoneticKeyboardLayout {
  ...
  static final Map<String, String> allSequences = _buildAllSequences();
  static final Map<String, List<String>> _sequencesByOutput =
      _buildSequencesByOutput();

  static Map<String, String> _buildAllSequences() {
    ...
    // Prefer official multi-key Avro sequences first.
    addAll(multiKeySequences);
    addAll(commonConjuncts);
    addAll(shiftKeys);
    addAll(normalKeys);
    return result;
  }

  static Map<String, List<String>> _buildSequencesByOutput() {
    final result = <String, List<String>>{};

    for (final entry in allSequences.entries) {
      final sequences = result.putIfAbsent(entry.value, () => <String>[]);
      if (!sequences.contains(entry.key)) {
        sequences.add(entry.key);
      }
    }

    for (final sequences in result.values) {
      sequences.sort((a, b) {
  ...
  static String? getOutputForSequence(String sequence) {
    return allSequences[sequence];
  }

  static List<MapEntry<String, String>> getMatchingEntriesStartingWith(
    String prefix,
  ) {
    final matches = allSequences.entries
        .where((entry) => entry.key.startsWith(prefix))
        .toList();

    matches.sort((a, b) => a.key.length.compareTo(b.key.length));
    return matches;
  }

  static List<String> getSequencesForOutput(String output) {
    return List<String>.from(_sequencesByOutput[output] ?? const <String>[]);
  }

  static String? getPrimarySequenceForOutput(String output) {
    final sequences = getSequencesForOutput(output);
    if (sequences.isEmpty) return null;

    final singleLetterLowercase = sequences.where(
      (sequence) =>
          sequence.length == 1 && sequence.toLowerCase() == sequence,
    );
    if (singleLetterLowercase.isNotEmpty) {
      return singleLetterLowercase.first;
    }

    final singleLetterNonAlphabetic = sequences.where(
      (sequence) =>
          sequence.length == 1 &&
          (sequence.toUpperCase() == sequence &&
              sequence.toLowerCase() == sequence),
    );
    if (singleLetterNonAlphabetic.isNotEmpty) {
      return singleLetterNonAlphabetic.first;
    }

    final singleLetterUppercase = sequences.where(
      (sequence) =>
          sequence.length == 1 && sequence.toUpperCase() == sequence,
    );
    if (singleLetterUppercase.isNotEmpty) {
      return singleLetterUppercase.first;
    }

    final multiLetterLowercase = sequences.where(
      (sequence) =>
          sequence.length > 1 && sequence.toLowerCase() == sequence,
    );
    if (multiLetterLowercase.isNotEmpty) {
      return multiLetterLowercase.first;
    }
  ...
  static List<List<String>> getDisplayRows() {
    return [
      // Physical key view for Windows Avro / phonetic typing
      ['`', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '-', '=', '⌫'],
      ['q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'],
      ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l'],
