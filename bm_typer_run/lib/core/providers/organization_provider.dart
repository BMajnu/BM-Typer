  ...
  return orgAsync.when(
    data: (org) {
      ...
      return Stream.fromFuture(
        service.ensurePrivilegedMemberAccess(org: org, user: user),
      ).asyncExpand((_) => service.getMembersStream(org.id));
    },
...
final teamMembersProvider = StreamProvider<List<OrgMemberModel>>((ref) {
  ...
  return orgAsync.when(
    data: (org) {
      ...
      return Stream.fromFuture(
        service.ensurePrivilegedMemberAccess(org: org, user: user),
      ).asyncExpand((_) => service.getMembersStream(org.id)).map((members) {
